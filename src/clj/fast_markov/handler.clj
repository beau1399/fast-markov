...
;(defn group[n] (fn [& v] (if (<= (count v) 1)(list v)(cons (take n v)(apply (group n) (rest v))))))
;(defn grp [n v] (let [frag (take n v) rest (drop n v)] (if (= (count rest) 0) (list v) (cons frag (grp n rest)))))
...
(def word-groups (group phrase-length (clojure.string/split food #" ")))
...
(def starters (map first (filter #(re-matches #"^[A-Z]{1}.*$" (first % ))  word-groups)))

;>(pick-words "I")
;("think" "it's" "important" "for")
(defn pick-words [p]
  (let [options (words-for p)] 
...
(defn make-quote
  ([] (make-quote (phrase)))
  ([p] (let [s (str p " " (pick-words (last(clojure.string/split p #"\s"))))]
     (if (= "_DOT_"  (last(clojure.string/split s #"\s")))
       (cleanup s)
       (recur s)))))
